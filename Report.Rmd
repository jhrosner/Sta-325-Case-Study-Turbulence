...
##### Inference
  ...
  We created several cubic polynomial models with different combinations of interaction terms. We chose the cubic polynomial models with interaction terms that resulted in the lowest MSE and Adjusted R squared values. Our final models for each of the moments for inference is a cubic polynomical with interaction terms between Re and Fr for all moments, Fr and St for the first moment, and Re and St for the last 3 moments.
$$\begin{aligned}
...
p1 = ggplot(prediction.data, aes(x = St, y = preds1, color = factor(Re))) +
  geom_point(size=0.5) +
  labs(title = "Polynomial Model - Moment 1", subtitle = "Fr*Re + Fr*St interaction")
p2 = ggplot(prediction.data, aes(x = St, y = preds2, color = factor(Re))) +
  geom_point(size=0.5) +
  labs(title = "Polynomial Model - Moment 2", subtitle = "Fr*Re + Re*St interaction")
p3 = ggplot(prediction.data, aes(x = St, y = preds3, color = factor(Re))) +
  geom_point(size=0.5) +
  labs(title = "Polynomial Model - Moment 3", subtitle = "Fr*Re + Re*St interaction")
p4 = ggplot(prediction.data, aes(x = St, y = preds4, color = factor(Re))) +
  geom_point(size=0.5) +
  labs(title = "Polynomial Model - Moment 4", subtitle = "Fr*Re + Re*St interaction")
...
kable(cv_results_final, digits=3, caption = "Cubic polynomial 5-fold CV")%>%
...
##### Inference Results
  ...
  Fr and Re alone both have a negative effect on all four log expected moments, but, the interaction effect of the two variables indicates that as they on larger values at the same time, the coefficients then take a positive effect instead, partially opposing the coefficient of the original negative effect. While interaction effects are usually less interpretable, in this case it makes sense to keep them in the final model. This is because high speeds cause turbulent flow, so the effect of turbulence (Re) on the distribution is different depending on the gravitational acceleration.For raw moment 1, this suggests that smoother flow at any level of gravitational acceleration may allow larger clusters to form, as they are not broken up by chaotic turbulence. For central moments 2, 3, and 4, this suggests that smoother flow with at any gravitational acceleration seemed to have a positive association with variance, skew, and kurtosis. Finally, the effects of particle size on each moment seems to significantly increase as our moment number increases.
  
  We also noticed that the interaction term log(St):Re improved the models Adjusted R-squared and RMSE values for moments 2, 3, and 4. The interaction term log(St):Re(398) was significant across the models for moments 2, 3, and 4. This interaction term has a negative association with variance, skew, and kurtosis. Thus, greater turbulent flow, Re, and larger log particle size, log(St), is associated with slightly lower variance, skew, and kurtosis. The interaction term log(St):Re was insignificant in our model for raw moment 1. However, the interaction term log(St):Fr improved the adjusted R-squared of the raw moment 1 model, and was significant across all levels of Fr with $\alpha=0.1$. For raw moment 1, larger log(St) values and Fr levels are associated with larger values of log mean particle cluster volume (first raw moment). 
  
  The coefficient $log(St)^2$ in our inference model for raw moment 1 was insignificant at the $\alpha=0.1$ level. This is a source of uncertainty in our model. Furthermore, Fr and Re were used as categorical variables in our models despite that they take on numeric values in practice. This approach limits the generlizability of our model, restricting its application to values within the categories. Lastly, no observations in the training set had both Re = 398 and Fr = 0.3, so that interaction term was not calculated by R. This could limit the application of our models at observations with those values.

##### Prediction Results

The figure below illustrates the final predictive results with standard errors on the hold-out testing dataset. As we mentioned in previous sections, the cubic spline model with an interaction between Re and Fr is the final prediction model that gave the best predictive results with the highest adjusted $R^2$ value and lowest MSE during validation set testing.

To estimate the uncertainty levels, we analyze the prediction standard errors as we did in the training process. As we expect, the standard errors are relatively small for first/second moments and become larger for third/fourth moments. This means that the true regression functions are more complicate for higher order moments, and this is expected because higher moments measure more innate features of the volume distribution. So models with higher flexibility may perform better for higher order moments.
...
st_p1 <- ggplot(pred_df, aes(x = St, y = R_moment_1)) +
  geom_point() +
  geom_errorbar(aes(ymin=R_moment_1-SE1, ymax=R_moment_1+SE1), color="red")+ 
  labs(title = "Raw moment 1 predictions", subtitle = "Spline with Fr*Re interaction")
st_p2 <- ggplot(pred_df, aes(x = St, y = R_moment_2)) +
  geom_point() +
  geom_errorbar(aes(ymin=R_moment_2-SE2, ymax=R_moment_2+SE2), color="red")+ 
  labs(title = "Raw moment 2 predictions", subtitle = "Spline with Fr*Re interaction")
st_p3 <- ggplot(pred_df, aes(x = St, y = R_moment_3)) +
  geom_point() +
  geom_errorbar(aes(ymin=R_moment_3-SE3, ymax=R_moment_3+SE3), color="red")+ 
  labs(title = "Raw moment 3 predictions", subtitle = "Spline with Fr*Re interaction")
st_p4 <- ggplot(pred_df, aes(x = St, y = R_moment_4)) +
  geom_point() +
  geom_errorbar(aes(ymin=R_moment_4-SE4, ymax=R_moment_4+SE4), color="red")+ 
  labs(title = "Raw moment 4 predictions", subtitle = "Spline with Fr*Re interaction")
