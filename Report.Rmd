...
$$\begin{aligned}
log(R_{moment1})=-1.486 + 5.599(log(St)) + 2.294(log(St))^2 + 1.167(log(St))^3 \\
-3.824(Re_{224})-6.015(Re_{398})--0.442(Fr_{0.3})-0.429(Fr_{\infty}) \\
+0.238(Re_{224}*Fr_{0.3}) +0.389(Re_{224}*Fr_{\infty})+0.504(Re_{398}*Fr_{\infty})
\end{aligned}$$

$$\begin{aligned}
log(C_{moment2})=6.005+9.517(log(St)) + 5.599(log(St))^2 + 3.907(log(St))^3 \\
-3.825(Re_{224})-6.005(Re_{398})-6.679(Fr_{0.3})-6.562(Fr_{\infty}) \\
...
#### Prediction
  ...
  To obtain our final model and analyze its predictive performance, we performed 5-fold cross validation on our GAM model using cubic splines with an interaction between Fr and Re, and obtained the final model output with the best parameters which were selected by cross validation. This GAM model outperformed any other model we created thus far, as it had extremely high $R^2$ values and it produced lower test MSEs for all four R moments as compared to any other models. As such, we conclude our search for and training of our predictive model, and move on to discuss the results we found from our final predictive GAM model. 
